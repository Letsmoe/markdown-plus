...
function editSummaryFile(file: string) {
	...
	var { html, markdown, metadata, deps } = compiler.compile(
		content,
		file,
		shared.env
	);
		...
		.on("all", function (event, file) {
			...
			if (
				...
				fs.writeFileSync(
					fullPath,
					config.backend == "html"
						? `<!DOCTYPE html><html><head><link rel="stylesheet" type="text/css" href="/style/main.css">${generateMetadata(
								metadata
						  )}</head><body><div id="sidebar" class="sidebar"></div><div class="inner-body">${html}</div></body><script src="/scripts/docs.js"></script><script src="/scripts/summary.js"></script></html>`
						: markdown
				);
