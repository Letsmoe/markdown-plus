...
const sha256 = (str: any) => crypto.createHash('md5').update(str, "utf-8").digest('hex');
...
let compiler: MarkdownPlusCompiler;
if (args.project) {
	...
	const runSingleFile = (file: string, output: string) => {
		if (!output) {
		...
		let { html, metadata, markdown } = compiler.compile(
			file,
			true,
			shared.env
		);
