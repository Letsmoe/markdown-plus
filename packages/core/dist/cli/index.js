...
function changeTargets(currentFile, deps, content) {
    /**
    ...
    for (const dependency of deps) {
        ...
        if (!target.match(root)) {
            ...
            if (fs.existsSync(target)) {
                ...
                content = content.replace(new RegExp(targetFile, "g"), newTarget.replace(out, ""));
            }
        }
...
function editSummaryFile(file) {
    ...
    var { markdown, dependencies } = compiler.compile(content, file, shared.env);
    markdown = changeTargets(file, dependencies, markdown);
    fs.writeFileSync(path.join(scripts, "summary.js"), `const s=\`${markdown.replace(/`/g, "\\`")}\`; document.getElementById("sidebar").innerHTML = s;`);
}
if (config.watch) {
    chokidar
