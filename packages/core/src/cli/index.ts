...
import { MarkdownPlusCompiler, DependencyGraph } from "../index.js";
import { Config, defaultConfig, validateConfig } from "../config.js";
...
import { Dependency } from "../dependencies/dependencies.type.js";
...
const sha256 = (str: any) =>
	crypto.createHash("md5").update(str, "utf-8").digest("hex");

function isExcludePath(file: string, config: Config): boolean {
...
if (!args.project) {
	error(
		"You need to specify a project to use, compiling single files is not yet possible."
	);
}
...
fs.writeFileSync(
	path.join(scripts, "docs.js"),
	`const p=${JSON.stringify(
		shared.scripts
	)};for(let e in p){let l=p[e];Array.from(document.querySelectorAll(\`code.\${e}\`)).length>0&&import(l).then((e=>{e.default(document)}))}`
);
		...
		if (!target.match(root)) {
			...
			if (fs.existsSync(target)) {
				...
				content = content.replace(
					new RegExp(targetFile, "g"),
					newTarget.replace(out, "")
				);
			}
		} else if (targetFile.endsWith(".mpp")) {
			// Rewrite the target to be of the output type (html, md...)
			let newTarget = changeExtension(
				targetFile,
				config.backend == "html" ? "html" : "md"
			);
			let reg = new RegExp(targetFile, "g");
			content = content.replace(reg, newTarget);
		}
	}
	return content;
}

function editSummaryFile(file: string) {
	let content = fs.readFileSync(file, "utf8");
	var { markdown, dependencies } = compiler.compile(
		content,
		file,
		shared.env
	);

	markdown = changeTargets(file, dependencies, markdown);

	fs.writeFileSync(
		path.join(scripts, "summary.js"),
...
if (config.watch) {
	chokidar
		...
		.on("all", function (event, file) {
			...
			if (
				!isExcludePath(file, config) &&
				(file.endsWith("md") || file.endsWith("mpp"))
			) {
				let date = new Date();
				let time = `[${pad(date.getHours())}:${pad(
					date.getMinutes()
				)}:${pad(date.getSeconds())}]`;
				let cStart = Date.now();
				console.clear();
				if (config.serve) {
					console.log(
						"Opened server on http://localhost:" +
				...
				const rewrittenPath = changeExtension(
					relPath,
					config.backend == "html" ? "html" : "md"
				);
				...
				fs.writeFileSync(
					fullPath,
					config.backend == "html"
						? `<!DOCTYPE html><html><head><link rel="stylesheet" type="text/css" href="/style/main.css"></head><body><div id="sidebar" class="sidebar"></div><div class="inner-body"></div></body><script src="/scripts/docs.js"></script><script src="/scripts/summary.js"></script></html>`
						: markdown
				);
...
function importPlayground(file: string): [string, string] {
