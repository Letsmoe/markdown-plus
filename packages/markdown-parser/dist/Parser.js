...
function Parser(lexer) {
    ...
    function listItem(title, ref = "", indent) {
        return {
            type: "listItem",
            title,
            ref,
    ...
    function createAST(statements) {
        const listRefs = { 0: list() };
        statements.forEach((v, i, arr) => {
    ...
    function parse() {
        ...
        return {
            files: statements.filter((t) => ["link"].includes(t.type)),
    ...
    function expression(numIndent = 0) {
        const token = lexer.consume();
        switch (token.type) {
            case TokenType.DASH:
    ...
    function parseListItem(numIndent = 0) {
        if (lexer.peek(0) === TokenType.LEFT_BRACE &&
