...
function run() {
    ...
    for (const file of files) {
        ...
        if (shared.config.compilerOptions.outputHTML) {
            ...
            content = content.replace(/<head>(.*?)<\/head>/gms, `<head><link rel="stylesheet" href="${shared.config.css}">$1</head>`);
        }
    ...
    let timeTaken = color(`${Math.round((performance.now() - cStart) * 10) / 10}ms`, COLORS.YELLOW);
